...
@objc(KeepChartMarkerView)
open class KeepMarkerView: NSUIView, KeepIMarker
{
    open var offset: CGPoint = CGPoint()
    
    @objc open weak var chartView: KeepChartViewBase?
    ...
    open func refreshContent(entry: KeepChartDataEntry, highlight: KeepHighlight)
    {
        // Do nothing here...
    }
    ...
    @objc
    open class func viewFromXib(in bundle: Bundle = .main) -> KeepMarkerView?
    {
        #if !os(OSX)
            return bundle.loadNibNamed(
                String(describing: self),
                owner: nil,
                options: nil)?[0] as? KeepMarkerView
        #else
            ...
            {
                return loadedObjects[0] as? KeepMarkerView
            }
