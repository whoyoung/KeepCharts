...
open class KeepAxisBase: KeepComponentBase
{
    ...
    private var _axisValueFormatter:  KeepIAxisValueFormatter?
    ...
    private var _limitLines = [KeepChartLimitLine]()
    ...
    @objc open var valueFormatter:  KeepIAxisValueFormatter?
    {
        get
        {
            if _axisValueFormatter == nil ||
                (_axisValueFormatter is KeepDefaultAxisValueFormatter &&
                    (_axisValueFormatter as! KeepDefaultAxisValueFormatter).hasAutoDecimals &&
                    (_axisValueFormatter as! KeepDefaultAxisValueFormatter).decimals != decimals)
            {
                _axisValueFormatter = KeepDefaultAxisValueFormatter(decimals: decimals)
            }
        ...
        set
        {
            _axisValueFormatter = newValue ?? KeepDefaultAxisValueFormatter(decimals: decimals)
        }
    }
    ...
    /// Adds a new KeepChartLimitLine to this axis.
    @objc open func addLimitLine(_ line: KeepChartLimitLine)
    {
        _limitLines.append(line)
    }
    
    /// Removes the specified KeepChartLimitLine from the axis.
    @objc open func removeLimitLine(_ line: KeepChartLimitLine)
    {
    ...
    @objc open var limitLines : [KeepChartLimitLine]
    {
        return _limitLines
    }
    ...
    /// Calculates the minimum, maximum and range values of the KeepYAxis with the given minimum and maximum values from the chart data.
