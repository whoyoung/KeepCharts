...
open class KeepChartHighlighter : NSObject, KeepIHighlighter
{
    /// instance of the data-provider
    @objc open weak var chart: KeepChartDataProvider?
    
    @objc public init(chart: KeepChartDataProvider)
    {
        self.chart = chart
    }
    
    open func getHighlight(x: CGFloat, y: CGFloat) -> KeepHighlight?
    {
    ...
    {
        guard let chart = self.chart as? KeepBarLineScatterCandleBubbleChartDataProvider else { return .zero }
    ...
    /// - parameter y:
    /// - returns:
    @objc open func getHighlight(xValue xVal: Double, x: CGFloat, y: CGFloat) -> KeepHighlight?
    {
    ...
    /// - returns: A list of KeepHighlight objects representing the entries closest to the given xVal.
    ...
    /// - returns:
    @objc open func getHighlights(xValue: Double, x: CGFloat, y: CGFloat) -> [KeepHighlight]
    {
    ...
    internal func buildHighlights(
        dataSet set: KeepIChartDataSet,
        dataSetIndex: Int,
        xValue: Double,
        rounding: KeepChartDataSetRounding) -> [KeepHighlight]
    {
        var highlights = [KeepHighlight]()
        
        guard let chart = self.chart as? KeepBarLineScatterCandleBubbleChartDataProvider else { return highlights }
        ...
        for e in entries
        {
            ...
            let highlight = KeepHighlight(x: e.x, y: e.y, xPx: px.x, yPx: px.y, dataSetIndex: dataSetIndex, axis: set.axisDependency)
    ...
    internal func getHighlightPos(high: KeepHighlight) -> CGFloat
    {
        return high.yPx
    }
    ...
    internal var data: KeepChartData?
    {
        return chart?.data
    }
}
