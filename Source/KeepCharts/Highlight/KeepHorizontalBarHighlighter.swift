//
//  KeepHorizontalBarHighlighter.swift
//  Charts
//
...
@objc(KeepHorizontalBarChartHighlighter)
open class KeepHorizontalBarHighlighter: KeepBarHighlighter
{
    open override func getHighlight(x: CGFloat, y: CGFloat) -> KeepHighlight?
    {
        guard let barData = self.chart?.data as? KeepBarChartData else { return nil }
        ...
        rounding: KeepChartDataSetRounding) -> [KeepHighlight]
    {
        ...
        for e in entries
        {
            ...
            highlights.append(KeepHighlight(x: e.x, y: e.y, xPx: px.x, yPx: px.y, dataSetIndex: dataSetIndex, axis: set.axisDependency))
        }
