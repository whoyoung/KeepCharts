//
//  KeepCandleStickChartRenderer.swift
//  Charts
//
...
open class KeepCandleStickChartRenderer: KeepLineScatterCandleRadarRenderer
{
    @objc open weak var dataProvider: KeepCandleChartDataProvider?
    
    @objc public init(dataProvider: KeepCandleChartDataProvider, animator: KeepAnimator, viewPortHandler: KeepViewPortHandler)
    {
    ...
    open override func drawData(context: CGContext)
    {
        ...
        for set in candleData.dataSets as! [KeepICandleChartDataSet]
        {
            if set.isVisible
            {
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: KeepICandleChartDataSet)
    {
        ...
        {
            // get the entry
            guard let e = dataSet.entryForIndex(j) as? KeepCandleChartDataEntry else { continue }
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        if isDrawingValuesAllowed(dataProvider: dataProvider)
        {
            ...
            {
                guard let dataSet = dataSets[i] as? KeepIBarLineScatterCandleBubbleChartDataSet
                    else { continue }
                ...
                for j in stride(from: _xBounds.min, through: _xBounds.range + _xBounds.min, by: 1)
                {
                    guard let e = dataSet.entryForIndex(j) as? KeepCandleChartDataEntry else { break }
    ...
    open override func drawHighlighted(context: CGContext, indices: [KeepHighlight])
    {
        ...
        {
            guard
                let set = candleData.getDataSetByIndex(high.dataSetIndex) as? KeepICandleChartDataSet,
                set.isHighlightEnabled
                else { continue }
            
            guard let e = set.entryForXValue(high.x, closestToY: high.y) as? KeepCandleChartDataEntry else { continue }
