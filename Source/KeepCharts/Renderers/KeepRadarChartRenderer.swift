...
open class KeepRadarChartRenderer: KeepLineRadarRenderer
{
    @objc open weak var chart: KeepRadarChartView?

    @objc public init(chart: KeepRadarChartView, animator: KeepAnimator, viewPortHandler: KeepViewPortHandler)
    {
    ...
    open override func drawData(context: CGContext)
    {
        ...
        if radarData != nil
        {
            ...
            for set in radarData!.dataSets as! [KeepIRadarChartDataSet]
            {
                if set.isVisible
                {
    ...
    internal func drawDataSet(context: CGContext, dataSet: KeepIRadarChartDataSet, mostEntries: Int)
    {
        guard let chart = chart else { return }
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        {
            let dataSet = data.getDataSetByIndex(i) as! KeepIRadarChartDataSet
    ...
    open override func drawHighlighted(context: CGContext, indices: [KeepHighlight])
    {
        guard
            let chart = chart,
            let radarData = chart.data as? KeepRadarChartData
            else { return }
        ...
        {
            guard
                let set = chart.data?.getDataSetByIndex(high.dataSetIndex) as? KeepIRadarChartDataSet,
                set.isHighlightEnabled
                else { continue }
            
            guard let e = set.entryForIndex(Int(high.x)) as? KeepRadarChartDataEntry
                else { continue }
