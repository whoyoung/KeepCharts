...
open class KeepLineChartRenderer: KeepLineRadarRenderer
{
    @objc open weak var dataProvider: KeepLineChartDataProvider?
    
    @objc public init(dataProvider: KeepLineChartDataProvider, animator: KeepAnimator, viewPortHandler: KeepViewPortHandler)
    {
    ...
    open override func drawData(context: CGContext)
    {
        ...
        for i in 0 ..< lineData.dataSetCount
        {
            ...
            if set.isVisible
            {
                if !(set is KeepILineChartDataSet)
                {
                    fatalError("Datasets for KeepLineChartRenderer must conform to KeepILineChartDataSet")
                }
                
                drawDataSet(context: context, dataSet: set as! KeepILineChartDataSet)
            }
        }
    }
    
    @objc open func drawDataSet(context: CGContext, dataSet: KeepILineChartDataSet)
    {
    ...
    @objc open func drawCubicBezier(context: CGContext, dataSet: KeepILineChartDataSet)
    {
        ...
        if _xBounds.range >= 1
        {
            ...
            var prevPrev: KeepChartDataEntry! = nil
            var prev: KeepChartDataEntry! = dataSet.entryForIndex(max(firstIndex - 2, 0))
            var cur: KeepChartDataEntry! = dataSet.entryForIndex(max(firstIndex - 1, 0))
            var next: KeepChartDataEntry! = cur
            var nextIndex: Int = -1
    ...
    @objc open func drawHorizontalBezier(context: CGContext, dataSet: KeepILineChartDataSet)
    {
        guard let dataProvider = dataProvider else { return }
        ...
        if _xBounds.range >= 1
        {
            var prev: KeepChartDataEntry! = dataSet.entryForIndex(_xBounds.min)
            var cur: KeepChartDataEntry! = prev
    ...
    @objc open func drawLinear(context: CGContext, dataSet: KeepILineChartDataSet)
    {
        guard let dataProvider = dataProvider else { return }
        ...
        if dataSet.colors.count > 1
        {
            ...
            {
                var e: KeepChartDataEntry! = dataSet.entryForIndex(j)
        ...
        { // only one color per dataset
            ...
            var e1: KeepChartDataEntry!
            var e2: KeepChartDataEntry!
    ...
    open func drawLinearFill(context: CGContext, dataSet: KeepILineChartDataSet, trans: KeepTransformer, bounds: XBounds)
    {
        guard let dataProvider = dataProvider else { return }
    ...
    private func generateFilledPath(dataSet: KeepILineChartDataSet, fillMin: CGFloat, bounds: XBounds, matrix: CGAffineTransform) -> CGPath
    {
        let phaseY = animator.phaseY
        let isDrawSteppedEnabled = dataSet.mode == .stepped
        let matrix = matrix
        
        var e: KeepChartDataEntry!
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        if isDrawingValuesAllowed(dataProvider: dataProvider)
        {
            ...
            {
                guard let dataSet = dataSets[i] as? KeepILineChartDataSet else { continue }
                ...
                for j in stride(from: _xBounds.min, through: min(_xBounds.min + _xBounds.range, _xBounds.max), by: 1)
                {
                    ...
                    {
                        KeepChartUtils.drawImage(context: context,
                                             image: icon,
    ...
    private func drawCircles(context: CGContext)
    {
        ...
        for i in 0 ..< dataSets.count
        {
            guard let dataSet = lineData.getDataSetByIndex(i) as? KeepILineChartDataSet else { continue }
    ...
    open override func drawHighlighted(context: CGContext, indices: [KeepHighlight])
    {
        ...
        for high in indices
        {
            guard let set = lineData.getDataSetByIndex(high.dataSetIndex) as? KeepILineChartDataSet
