...
open class KeepBubbleChartRenderer: KeepBarLineScatterCandleBubbleRenderer
{
    @objc open weak var dataProvider: KeepBubbleChartDataProvider?
    
    @objc public init(dataProvider: KeepBubbleChartDataProvider, animator: KeepAnimator, viewPortHandler: KeepViewPortHandler)
    {
    ...
    open override func drawData(context: CGContext)
    {
        ...
        for set in bubbleData.dataSets as! [KeepIBubbleChartDataSet] where set.isVisible
        {
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: KeepIBubbleChartDataSet)
    {
        ...
        {
            guard let entry = dataSet.entryForIndex(j) as? KeepBubbleChartDataEntry else { continue }
    ...
    open override func drawValues(context: CGContext)
    {
        guard let
            ...
            let dataSets = bubbleData.dataSets as? [KeepIBubbleChartDataSet]
            else { return }
        ...
        for i in 0..<dataSets.count
        {
            ...
            {
                guard let e = dataSet.entryForIndex(j) as? KeepBubbleChartDataEntry else { break }
    ...
    open override func drawHighlighted(context: CGContext, indices: [KeepHighlight])
    {
        ...
        {
            guard
                let dataSet = bubbleData.getDataSetByIndex(high.dataSetIndex) as? KeepIBubbleChartDataSet,
                dataSet.isHighlightEnabled,
                let entry = dataSet.entryForXValue(high.x, closestToY: high.y) as? KeepBubbleChartDataEntry,
