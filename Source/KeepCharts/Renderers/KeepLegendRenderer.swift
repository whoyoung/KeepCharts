...
@objc(KeepChartLegendRenderer)
open class KeepLegendRenderer: KeepRenderer
{
    /// the legend object this renderer renders
    @objc open var legend: KeepLegend?

    @objc public init(viewPortHandler: KeepViewPortHandler, legend: KeepLegend?)
    {
    ...
    @objc open func computeLegend(data: KeepChartData)
    {
        guard let legend = legend else { return }
        
        if !legend.isLegendCustom
        {
            var entries: [KeepLegendEntry] = []
            ...
            for i in 0..<data.dataSetCount
            {
                ...
                if dataSet is KeepIBarChartDataSet &&
                    (dataSet as! KeepIBarChartDataSet).isStacked
                {
                    let bds = dataSet as! KeepIBarChartDataSet
                ...
                else if dataSet is KeepIPieChartDataSet
                {
                    let pds = dataSet as! KeepIPieChartDataSet
                    
                    for j in 0..<min(clrs.count, entryCount)
                    {
                        entries.append(
                            KeepLegendEntry(
                                label: (pds.entryForIndex(j) as? KeepPieChartDataEntry)?.label,
                                form: dataSet.form,
                                formSize: dataSet.formSize,
                                formLineWidth: dataSet.formLineWidth,
                                formLineDashPhase: dataSet.formLineDashPhase,
                ...
                else if dataSet is KeepICandleChartDataSet &&
                    (dataSet as! KeepICandleChartDataSet).decreasingColor != nil
                {
                    let candleDataSet = dataSet as! KeepICandleChartDataSet
    ...
    /// Draws the KeepLegend-form at the given position with the color at the given index.
    @objc open func drawForm(
        context: CGContext,
        x: CGFloat,
        y: CGFloat,
        entry: KeepLegendEntry,
        legend: KeepLegend)
    {
        guard
    ...
    {
        KeepChartUtils.drawText(context: context, text: label, point: CGPoint(x: x, y: y), align: .left, attributes: [NSAttributedStringKey.font: font, NSAttributedStringKey.foregroundColor: textColor])
    }
}
