...
open class KeepBarChartRenderer: KeepBarLineScatterCandleBubbleRenderer
{
    ...
    @objc open weak var dataProvider: KeepBarChartDataProvider?
    
    @objc public init(dataProvider: KeepBarChartDataProvider, animator: KeepAnimator, viewPortHandler: KeepViewPortHandler)
    {
    ...
    open override func initBuffers()
    {
        if let barData = dataProvider?.barData
        {
            ...
            {
                let set = barData.dataSets[i] as! KeepIBarChartDataSet
    ...
    private func prepareBuffer(dataSet: KeepIBarChartDataSet, index: Int)
    {
        guard
        ...
        {
            guard let e = dataSet.entryForIndex(i) as? KeepBarChartDataEntry else { continue }
    ...
    open override func drawData(context: CGContext)
    {
        ...
        for i in 0 ..< barData.dataSetCount
        {
            ...
            if set.isVisible
            {
                if !(set is KeepIBarChartDataSet)
                {
                    fatalError("Datasets for KeepBarChartRenderer must conform to IBarChartDataset")
                }
                
                drawDataSet(context: context, dataSet: set as! KeepIBarChartDataSet, index: i)
            }
        }
    }
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: KeepIBarChartDataSet, index: Int)
    {
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        if isDrawingValuesAllowed(dataProvider: dataProvider)
        {
            ...
            {
                guard let dataSet = dataSets[dataSetIndex] as? KeepIBarChartDataSet else { continue }
                ...
                if !dataSet.isStacked
                {
                    ...
                    {
                        guard let e = dataSet.entryForIndex(j) as? KeepBarChartDataEntry else { continue }
                ...
                else
                {
                    ...
                    {
                        guard let e = dataSet.entryForIndex(index) as? KeepBarChartDataEntry else { continue }
    ...
    {
        KeepChartUtils.drawText(context: context, text: value, point: CGPoint(x: xPos, y: yPos), align: align, attributes: [NSAttributedStringKey.font: font, NSAttributedStringKey.foregroundColor: color])
    }
    ...
    open override func drawHighlighted(context: CGContext, indices: [KeepHighlight])
    {
        ...
        {
            guard
                let set = barData.getDataSetByIndex(high.dataSetIndex) as? KeepIBarChartDataSet,
                set.isHighlightEnabled
                else { continue }
            
            if let e = set.entryForXValue(high.x, closestToY: high.y) as? KeepBarChartDataEntry
            {
