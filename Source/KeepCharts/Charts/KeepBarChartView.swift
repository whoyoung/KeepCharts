...
open class KeepBarChartView: KeepBarLineChartViewBase, KeepBarChartDataProvider
{
    ...
    internal override func initialize()
    {
        ...
        renderer = KeepBarChartRenderer(dataProvider: self, animator: _animator, viewPortHandler: _viewPortHandler)
        
        self.highlighter = KeepBarHighlighter(chart: self)
    ...
    {
        guard let data = self.data as? KeepBarChartData
            else { return }
    ...
    /// - returns: The KeepHighlight object (contains x-index and DataSet index) of the selected value at the given touch point inside the BarChart.
    ...
    @objc open func getBarBounds(entry e: KeepBarChartDataEntry) -> CGRect
    {
        guard let
            data = _data as? KeepBarChartData,
    ...
    {
        highlightValue(KeepHighlight(x: x, dataSetIndex: dataSetIndex, stackIndex: stackIndex))
    }
    ...
    open var barData: KeepBarChartData? { return _data as? KeepBarChartData }
