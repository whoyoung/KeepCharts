//
//  KeepBarLineChartViewBase.swift
//  Charts
//
...
open class KeepBarLineChartViewBase: KeepChartViewBase, KeepBarLineScatterCandleBubbleChartDataProvider, NSUIGestureRecognizerDelegate
{
    ...
    /// top axis.
    @objc open internal(set) var leftAxis = KeepYAxis(position: .left)
    
    /// The right y-axis object. In the horizontal bar-chart, this is the
    /// bottom axis.
    @objc open internal(set) var rightAxis = KeepYAxis(position: .right)

    /// The left Y axis renderer. This is a read-write property so you can set your own custom renderer here.
    /// **default**: An instance of KeepYAxisRenderer
    @objc open lazy var leftYAxisRenderer = KeepYAxisRenderer(viewPortHandler: _viewPortHandler, yAxis: leftAxis, transformer: _leftAxisTransformer)

    /// The right Y axis renderer. This is a read-write property so you can set your own custom renderer here.
    /// **default**: An instance of KeepYAxisRenderer
    @objc open lazy var rightYAxisRenderer = KeepYAxisRenderer(viewPortHandler: _viewPortHandler, yAxis: rightAxis, transformer: _rightAxisTransformer)
    
    internal var _leftAxisTransformer: KeepTransformer!
    internal var _rightAxisTransformer: KeepTransformer!
    
    /// The X axis renderer. This is a read-write property so you can set your own custom renderer here.
    /// **default**: An instance of KeepXAxisRenderer
    @objc open lazy var xAxisRenderer = KeepXAxisRenderer(viewPortHandler: _viewPortHandler, xAxis: _xAxis, transformer: _leftAxisTransformer)
    ...
    internal override func initialize()
    {
        ...
        _leftAxisTransformer = KeepTransformer(viewPortHandler: _viewPortHandler)
        _rightAxisTransformer = KeepTransformer(viewPortHandler: _viewPortHandler)
        
        self.highlighter = KeepChartHighlighter(chart: self)
        ...
        _longTapGestureRecognizer = NSUILongPressGestureRecognizer(target: self, action: #selector(KeepBarLineChartViewBase.longTapGestureRecognized(_:)))
        ...
        #if !os(tvOS)
            _pinchGestureRecognizer = NSUIPinchGestureRecognizer(target: self, action: #selector(KeepBarLineChartViewBase.pinchGestureRecognized(_:)))
    ...
    private var _closestDataSetToTouch: KeepIChartDataSet!
    ...
    @objc private func pinchGestureRecognized(_ recognizer: NSUIPinchGestureRecognizer)
    {
            ...
            recognizer.state == NSUIGestureRecognizerState.cancelled
        {
            if _isScaling
            {
                ...
                // KeepRange might have changed, which means that Y-axis labels could have changed in size, affecting Y-axis size. So we need to recalculate offsets.
                calculateOffsets()
                setNeedsDisplay()
            }
        }
        ...
        else if recognizer.state == NSUIGestureRecognizerState.ended || recognizer.state == NSUIGestureRecognizerState.cancelled
        {
            if _isDragging
            {
                if recognizer.state == NSUIGestureRecognizerState.ended && isDragDecelerationEnabled
                {
                    ...
                    _decelerationDisplayLink = NSUIDisplayLink(target: self, selector: #selector(KeepBarLineChartViewBase.decelerationLoop))
    ...
    private func performPanChange(translation: CGPoint) -> Bool
    {
        ...
        if isTouchInverted()
        {
            if self is KeepHorizontalBarChartView
            {
        ...
        easing: KeepChartEasingFunctionBlock?)
    {
        ...
        let job = KeepAnimatedZoomViewJob(
            viewPortHandler: viewPortHandler,
            transformer: getTransformer(forAxis: axis),
            view: self,
            yAxis: getAxis(axis),
        ...
        easing: KeepChartEasingFunctionBlock?)
    {
        ...
        let job = KeepAnimatedMoveViewJob(
            viewPortHandler: viewPortHandler,
            xValue: xValue,
            yValue: yValue + yInView / 2.0,
            transformer: getTransformer(forAxis: axis),
    ...
    @objc open func getEntryByTouchPoint(point pt: CGPoint) -> KeepChartDataEntry!
    {
    ...
    @objc open func getDataSetByTouchPoint(point pt: CGPoint) -> KeepIBarLineScatterCandleBubbleChartDataSet?
    {
        let h = getHighlightByTouchPoint(pt)
        if h !== nil
        {
            return _data?.getDataSetByIndex(h!.dataSetIndex) as? KeepIBarLineScatterCandleBubbleChartDataSet
        }
        return nil
    }
    ...
    // MARK: - KeepBarLineScatterCandleBubbleChartDataProvider
