...
public protocol KeepChartViewDelegate
{
    ...
    @objc optional func chartValueSelected(_ chartView: KeepChartViewBase, entry: KeepChartDataEntry, highlight: KeepHighlight)
    
    @objc optional func chartValueLongPressed(_ chartView: KeepChartViewBase, entry: KeepChartDataEntry, dataSetIndex: Int, highlight: KeepHighlight)
    
    // Called when nothing has been selected or an "un-select" has been made.
    @objc optional func chartValueNothingSelected(_ chartView: KeepChartViewBase)
    
    // Callbacks when the chart is scaled / zoomed via pinch zoom gesture.
    @objc optional func chartScaled(_ chartView: KeepChartViewBase, scaleX: CGFloat, scaleY: CGFloat)
...
open class KeepChartViewBase: NSUIView, KeepChartDataProvider, KeepAnimatorDelegate
{
    ...
    /// acquire the KeepXAxis object and modify it (e.g. change the position of the
    /// labels)
    @objc open var xAxis: KeepXAxis
    {
        return _xAxis
    }
    
    /// The default KeepIValueFormatter that has been determined by the chart considering the provided minimum and maximum values.
    internal var _defaultValueFormatter: KeepIValueFormatter? = KeepDefaultValueFormatter(decimals: 0)
    
    /// object that holds all data that was originally set for the chart, before it was modified or any filtering algorithms had been applied
    internal var _data: KeepChartData?
    ...
    internal var _legendRenderer: KeepLegendRenderer!
    
    /// object responsible for rendering the data
    @objc open var renderer: KeepDataRenderer?
    
    @objc open var highlighter: KeepIHighlighter?
    
    /// object that manages the bounds and drawing constraints of the chart
    internal var _viewPortHandler: KeepViewPortHandler!
    
    /// object responsible for animations
    internal var _animator: KeepAnimator!
    
    /// flag that indicates if offsets calculation has already been done or not
    private var _offsetsCalculated = false
    
    /// array of KeepHighlight objects that reference the highlighted slices in the chart
    internal var _indicesToHighlight = [KeepHighlight]()
    ...
    @objc open var marker: KeepIMarker?
    ...
    internal func initialize()
    {
        ...
        _animator = KeepAnimator()
        _animator.delegate = self

        _viewPortHandler = KeepViewPortHandler(width: bounds.size.width, height: bounds.size.height)
        
        chartDescription = KeepDescription()
        
        _legend = KeepLegend()
        _legendRenderer = KeepLegendRenderer(viewPortHandler: _viewPortHandler, legend: _legend)
    ...
    {
        fatalError("notifyDataSetChanged() cannot be called on KeepChartViewBase")
    }
    
    /// Calculates the offsets of the chart to the border depending on the position of an eventual legend or depending on the length of the y-axis and x-axis labels and their position
    internal func calculateOffsets()
    {
        fatalError("calculateOffsets() cannot be called on KeepChartViewBase")
    }
    
    /// calcualtes the y-min and y-max value and the y-delta and x-delta value
    internal func calcMinMax()
    {
        fatalError("calcMinMax() cannot be called on KeepChartViewBase")
    }
    ...
    internal func setupDefaultFormatter(min: Double, max: Double)
    {
        ...
        if _defaultValueFormatter is KeepDefaultValueFormatter
        {
            // setup the formatter with a new number of digits
            let digits = reference.decimalPlaces
            
            (_defaultValueFormatter as? KeepDefaultValueFormatter)?.decimals
             = digits
        }
    }
    ...
    open override func draw(_ rect: CGRect)
    {
        ...
        if _data === nil && noDataText.count > 0
        {
            ...
            KeepChartUtils.drawMultilineText(
                context: context,
                text: noDataText,
    ...
    /// KeepHighlights the values at the given indices in the given DataSets. Provide
    /// null or an empty array to undo all highlighting. 
    /// This should be used to programmatically highlight values.
    /// This method *will not* call the delegate.
    @objc open func highlightValues(_ highs: [KeepHighlight]?)
    {
        // set the indices to highlight
        _indicesToHighlight = highs ?? [KeepHighlight]()
    ...
    /// KeepHighlights any y-value at the given x-value in the given DataSet.
    ...
    /// - parameter dataIndex: The data index to search in (only used in KeepCombinedChartView currently)
    ...
    /// KeepHighlights the value at the given x-value and y-value in the given DataSet.
    /// Provide -1 as the dataSetIndex to undo all highlighting.
    /// This method will call the delegate.
    /// - parameter x: The x-value to highlight
    ...
    @objc open func highlightValue(x: Double, y: Double, dataSetIndex: Int, dataIndex: Int = -1, callDelegate: Bool)
    {
        ...
        {
            highlightValue(KeepHighlight(x: x, y: y, dataSetIndex: dataSetIndex, dataIndex: dataIndex), callDelegate: callDelegate)
        }
    }
    
    /// KeepHighlights the values represented by the provided KeepHighlight object
    /// This method *will not* call the delegate.
    /// - parameter highlight: contains information about which entry should be highlighted
    @objc open func highlightValue(_ highlight: KeepHighlight?)
    {
        highlightValue(highlight, callDelegate: false)
    }

    open func highlightValueForLongTap(_ highlight: KeepHighlight?, callDelegate: Bool) {
        var entry: KeepChartDataEntry?
        let h = highlight
        if (h == nil) {
            return;
        } else {
    ...
    /// KeepHighlights the value selected by touch gesture.
    @objc open func highlightValue(_ highlight: KeepHighlight?, callDelegate: Bool)
    {
        var entry: KeepChartDataEntry?
        var h = highlight
    ...
    /// - returns: The KeepHighlight object (contains x-index and DataSet index) of the
    /// selected value at the given touch point inside the Line-, Scatter-, or
    /// CandleStick-Chart.
    @objc open func getHighlightByTouchPoint(_ pt: CGPoint) -> KeepHighlight?
    {
        if _data === nil
        {
    ...
    @objc open var lastHighlighted: KeepHighlight?
    ...
    /// - returns: The actual position in pixels of the KeepMarkerView for the given Entry in the given DataSet.
    @objc open func getMarkerPosition(highlight: KeepHighlight) -> CGPoint
    {
    ...
    @objc open var chartAnimator: KeepAnimator!
    {
        return _animator
    }
    ...
    /// - returns: The KeepLegend object of the chart. This method can be used to get an instance of the legend in order to customize the automatically generated KeepLegend.
    @objc open var legend: KeepLegend
    {
        return _legend
    }
    
    /// - returns: The renderer object responsible for rendering / drawing the KeepLegend.
    @objc open var legendRenderer: KeepLegendRenderer!
    {
    ...
    /// - returns: The KeepViewPortHandler of the chart that is responsible for the
    /// content area of the chart and its offsets and dimensions.
    @objc open var viewPortHandler: KeepViewPortHandler!
    {
    ...
    internal var _viewportJobs = [KeepViewPortJob]()
    ...
    @objc open func removeViewportJob(_ job: KeepViewPortJob)
    {
    ...
    @objc open func addViewportJob(_ job: KeepViewPortJob)
    {
    ...
    // MARK: - KeepAnimatorDelegate
    
    open func animatorUpdated(_ chartAnimator: KeepAnimator)
    {
        setNeedsDisplay()
    }
    
    open func animatorStopped(_ chartAnimator: KeepAnimator)
    {
