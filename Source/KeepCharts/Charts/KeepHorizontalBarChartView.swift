//
//  KeepHorizontalBarChartView.swift
//  Charts
//
...
open class KeepHorizontalBarChartView: KeepRenamedBarChartView
{
    internal override func initialize()
    {
        ...
        _leftAxisTransformer = KeepTransformerHorizontalBarChart(viewPortHandler: _viewPortHandler)
        _rightAxisTransformer = KeepTransformerHorizontalBarChart(viewPortHandler: _viewPortHandler)
        
        renderer = KeepHorizontalBarChartRenderer(dataProvider: self, animator: _animator, viewPortHandler: _viewPortHandler)
        leftYAxisRenderer = KeepYAxisRendererHorizontalBarChart(viewPortHandler: _viewPortHandler, yAxis: leftAxis, transformer: _leftAxisTransformer)
        rightYAxisRenderer = KeepYAxisRendererHorizontalBarChart(viewPortHandler: _viewPortHandler, yAxis: rightAxis, transformer: _rightAxisTransformer)
        xAxisRenderer = KeepXAxisRendererHorizontalBarChart(viewPortHandler: _viewPortHandler, xAxis: _xAxis, transformer: _leftAxisTransformer, chart: self)
        
        self.highlighter = KeepHorizontalBarHighlighter(chart: self)
    }
    ...
    open override func getMarkerPosition(highlight: KeepHighlight) -> CGPoint
    {
        return CGPoint(x: highlight.drawY, y: highlight.drawX)
    }
    
    open override func getBarBounds(entry e: KeepBarChartDataEntry) -> CGRect
    {
        guard
            let data = _data as? KeepBarChartData,
            let set = data.getDataSetForEntry(e) as? KeepIBarChartDataSet
    ...
    open override func getPosition(entry e: KeepChartDataEntry, axis: KeepYAxis.KeepAxisDependency) -> CGPoint
    {
    ...
    open override func setVisibleYRangeMaximum(_ maxYRange: Double, axis: KeepYAxis.KeepAxisDependency)
    {
        let yScale = getAxisRange(axis: axis) / maxYRange
        viewPortHandler.setMinimumScaleX(CGFloat(yScale))
    }
    
    open override func setVisibleYRangeMinimum(_ minYRange: Double, axis: KeepYAxis.KeepAxisDependency)
    {
        let yScale = getAxisRange(axis: axis) / minYRange
        viewPortHandler.setMaximumScaleX(CGFloat(yScale))
    }
    
    open override func setVisibleYRange(minYRange: Double, maxYRange: Double, axis: KeepYAxis.KeepAxisDependency)
    {
