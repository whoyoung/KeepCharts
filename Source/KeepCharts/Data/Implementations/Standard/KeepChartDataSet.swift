...
@objc
public enum KeepChartDataSetRounding: Int
{
    case up = 0
    case down = 1
    case closest = 2
...
open class KeepChartDataSet: KeepChartBaseDataSet
{
    ...
    @objc public init(values: [KeepChartDataEntry]?, label: String?)
    {
        self.values = values ?? []
    ...
    @objc public convenience init(values: [KeepChartDataEntry]?)
    {
    ...
    @objc open var values: [KeepChartDataEntry]
        {
        didSet
        {
    ...
    @objc open func calcMinMaxX(entry e: KeepChartDataEntry)
    {
        if e.x < _xMin
        {
            _xMin = e.x
    ...
    @objc open func calcMinMaxY(entry e: KeepChartDataEntry)
    {
        if e.y < _yMin
        {
            _yMin = e.y
    ...
    ///
    /// - parameter e:
    internal func calcMinMax(entry e: KeepChartDataEntry)
    {
        calcMinMaxX(entry: e)
        calcMinMaxY(entry: e)
    }
    ...
    open override func entryForIndex(_ i: Int) -> KeepChartDataEntry?
    {
    ...
    open override func entryForXValue(
        _ xValue: Double,
        closestToY yValue: Double) -> KeepChartDataEntry?
    {
    ...
    open override func entriesForXValue(_ xValue: Double) -> [KeepChartDataEntry]
    {
        var entries = [KeepChartDataEntry]()
    ...
    open override func entryIndex(entry e: KeepChartDataEntry) -> Int
    {
    ...
    /// - returns: True
    open override func addEntry(_ e: KeepChartDataEntry) -> Bool
    {
        calcMinMax(entry: e)
    ...
    /// - parameter e: the entry to add
    /// - returns: True
    open override func addEntryOrdered(_ e: KeepChartDataEntry) -> Bool
    {
        calcMinMax(entry: e)
    ...
    open override func removeEntry(_ entry: KeepChartDataEntry) -> Bool
    {
        var removed = false
        isIndirectValuesCall = true
    ...
    {
        let entry: KeepChartDataEntry? = values.isEmpty ? nil : values.removeFirst()
        return entry != nil
    }
    ...
    {
        let entry: KeepChartDataEntry? = values.isEmpty ? nil : values.removeLast()
        return entry != nil
    }
    
    /// Checks if this DataSet contains the specified Entry.
    /// - returns: `true` if contains the entry, `false` if not.
    open override func contains(_ e: KeepChartDataEntry) -> Bool
    {
        for entry in values
        {
            if entry == e
    ...
    {
        let copy = super.copyWithZone(zone) as! KeepChartDataSet
