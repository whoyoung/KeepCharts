...
open class KeepChartData: NSObject
{
    ...
    internal var _dataSets = [KeepIChartDataSet]()
    
    public override init()
    {
        super.init()
        
        _dataSets = [KeepIChartDataSet]()
    }
    
    @objc public init(dataSets: [KeepIChartDataSet]?)
    {
        super.init()
        
        _dataSets = dataSets ?? [KeepIChartDataSet]()
        
        self.initialize(dataSets: _dataSets)
    }
    
    @objc public convenience init(dataSet: KeepIChartDataSet?)
    {
        self.init(dataSets: dataSet === nil ? nil : [dataSet!])
    }
    
    internal func initialize(dataSets: [KeepIChartDataSet])
    {
        notifyDataChanged()
    }
    
    /// Call this method to let the KeepChartData know that the underlying data has changed.
    ...
    @objc open func calcMinMax(dataSet d: KeepIChartDataSet)
    {
        if _yMax < d.yMax
        {
            _yMax = d.yMax
    ...
    @objc open var dataSets: [KeepIChartDataSet]
    {
        get
        {
            return _dataSets
    ...
    /// Retrieve the index of a KeepChartDataSet with a specific label from the KeepChartData. Search can be case sensitive or not.
    /// 
    ...
    @objc open func entryForHighlight(_ highlight: KeepHighlight) -> KeepChartDataEntry?
    {
    ...
    @objc open func getDataSetByLabel(_ label: String, ignorecase: Bool) -> KeepIChartDataSet?
    {
    ...
    @objc open func getDataSetByIndex(_ index: Int) -> KeepIChartDataSet!
    {
        if index < 0 || index >= _dataSets.count
        {
            return nil
    ...
    @objc open func addDataSet(_ dataSet: KeepIChartDataSet!)
    {
    ...
    @objc @discardableResult open func removeDataSet(_ dataSet: KeepIChartDataSet!) -> Bool
    {
        if dataSet === nil
        {
            return false
    ...
    @objc open func addEntry(_ e: KeepChartDataEntry, dataSetIndex: Int)
    {
    ...
    @objc @discardableResult open func removeEntry(_ entry: KeepChartDataEntry, dataSetIndex: Int) -> Bool
    {
        // entry outofbounds
    ...
    /// Removes the Entry object closest to the given xIndex from the KeepChartDataSet at the
    /// specified index. 
    ...
    @objc open func getDataSetForEntry(_ e: KeepChartDataEntry!) -> KeepIChartDataSet?
    {
        if e == nil
        {
            return nil
    ...
    @objc open func indexOfDataSet(_ dataSet: KeepIChartDataSet) -> Int
    {
        for i in 0 ..< _dataSets.count
        {
            if _dataSets[i] === dataSet
    ...
    @objc open func getFirstLeft(dataSets: [KeepIChartDataSet]) -> KeepIChartDataSet?
    {
    ...
    @objc open func getFirstRight(dataSets: [KeepIChartDataSet]) -> KeepIChartDataSet?
    {
        for dataSet in _dataSets
        {
            if dataSet.axisDependency == .right
    ...
    /// Sets a custom KeepIValueFormatter for all DataSets this data object contains.
    @objc open func setValueFormatter(_ formatter: KeepIValueFormatter?)
    {
    ...
    @objc open func contains(dataSet: KeepIChartDataSet) -> Bool
    {
        for set in dataSets
        {
            if set === dataSet
    ...
    @objc open var maxEntryCountSet: KeepIChartDataSet?
    {
