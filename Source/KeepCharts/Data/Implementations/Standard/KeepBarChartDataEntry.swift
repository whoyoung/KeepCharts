...
open class KeepBarChartDataEntry: KeepChartDataEntry
{
    /// the values the stacked barchart holds
    private var _yVals: [Double]?
    
    /// the ranges for the individual stack values - automatically calculated
    private var _ranges: [KeepRange]?
    ...
    {
        super.init(x: x, y: KeepBarChartDataEntry.calcSum(values: yValues))
        self._yVals = yValues
        calcPosNegSum()
        calcRanges()
    }
    ...
    {
        super.init(x: x, y: KeepBarChartDataEntry.calcSum(values: yValues), data: data)
        self._yVals = yValues
        calcPosNegSum()
        calcRanges()
    }
    ...
    {
        super.init(x: x, y: KeepBarChartDataEntry.calcSum(values: yValues), icon: icon, data: data)
        self._yVals = yValues
        calcPosNegSum()
        calcRanges()
    }
    ...
    {
        super.init(x: x, y: KeepBarChartDataEntry.calcSum(values: yValues), icon: icon)
        self._yVals = yValues
        calcPosNegSum()
        calcRanges()
    }
    ...
    /// Splits up the stack-values of the given bar-entry into KeepRange objects.
    /// - parameter entry:
    /// - returns:
    @objc open func calcRanges()
    {
        ...
        for i in 0 ..< values!.count
        {
            ...
            if value < 0
            {
                _ranges?.append(KeepRange(from: negRemain, to: negRemain - value))
                negRemain -= value
            }
            else
            {
                _ranges?.append(KeepRange(from: posRemain, to: posRemain + value))
                posRemain += value
            }
        }
    }
    ...
    @objc open var yValues: [Double]?
    {
        ...
        set
        {
            self.y = KeepBarChartDataEntry.calcSum(values: newValue)
    ...
    @objc open var ranges: [KeepRange]?
    {
        return _ranges
    }
    ...
    {
        let copy = super.copyWithZone(zone) as! KeepBarChartDataEntry
        copy._yVals = _yVals
        copy.y = y
