...
open class KeepBarChartDataSet: KeepBarLineScatterCandleBubbleChartDataSet, KeepIBarChartDataSet
{
    private func initialize()
    {
        self.highlightColor = NSUIColor.black
        
        self.calcStackSize(entries: values as! [KeepBarChartDataEntry])
        self.calcEntryCountIncludingStacks(entries: values as! [KeepBarChartDataEntry])
    }
    ...
    public override init(values: [KeepChartDataEntry]?, label: String?)
    {
    ...
    private func calcEntryCountIncludingStacks(entries: [KeepBarChartDataEntry])
    {
        _entryCountStacks = 0
    ...
    private func calcStackSize(entries: [KeepBarChartDataEntry])
    {
        for i in 0 ..< entries.count
        {
            if let vals = entries[i].yValues
    ...
    {
        guard let e = e as? KeepBarChartDataEntry
            else { return }
    ...
    {
        let copy = super.copyWithZone(zone) as! KeepBarChartDataSet
